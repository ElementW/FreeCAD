...
void MTextEdit::insertFromMimeData(const QMimeData *source) {
    if (source->hasImage()) {
        ...
        for (int i=0; i<formats.size(); i++) {
            if (formats[i] == QStringLiteral("image/bmp"))  { format = QString::fromLatin1("BMP");  break; }
            if (formats[i] == QStringLiteral("image/jpeg")) { format = QString::fromLatin1("JPG");  break; }
            if (formats[i] == QStringLiteral("image/jpg"))  { format = QString::fromLatin1("JPG");  break; }
            if (formats[i] == QStringLiteral("image/gif"))  { format = QString::fromLatin1("GIF");  break; }
            if (formats[i] == QStringLiteral("image/png"))  { format = QString::fromLatin1("PNG");  break; }
            if (formats[i] == QStringLiteral("image/pbm"))  { format = QString::fromLatin1("PBM");  break; }
            if (formats[i] == QStringLiteral("image/pgm"))  { format = QString::fromLatin1("PGM");  break; }
            if (formats[i] == QStringLiteral("image/ppm"))  { format = QString::fromLatin1("PPM");  break; }
            if (formats[i] == QStringLiteral("image/tiff")) { format = QString::fromLatin1("TIFF"); break; }
            if (formats[i] == QStringLiteral("image/xbm"))  { format = QString::fromLatin1("XBM");  break; }
            if (formats[i] == QStringLiteral("image/xpm"))  { format = QString::fromLatin1("XPM");  break; }
            }
        if (!format.isEmpty()) {
